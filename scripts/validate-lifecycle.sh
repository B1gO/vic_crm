...
echo "Scenario 2: Nina RESUME -> MOCKING without RESUME_READY (should fail)"
resp=$(request POST "/api/candidates/$nina_id/transition" '{"toStage":"MOCKING","reason":"test"}')
expect_status "Nina MOCKING without RESUME_READY" "$resp" "400"
...
echo "Scenario 7: Nina set RESUME_READY then RESUME -> MOCKING (should pass)"
resp=$(request POST "/api/candidates/$nina_id/substatus" '{"subStatus":"RESUME_READY","reason":"Resume ready"}')
expect_status "Nina substatus RESUME_READY" "$resp" "200"
resp=$(request POST "/api/candidates/$nina_id/transition" '{"toStage":"MOCKING","reason":"Resume ready"}')
expect_status "Nina MOCKING after resume ready" "$resp" "200"

echo "Scenario 8: Nina MOCKING -> MARKETING (should fail)"
...
echo "Scenario 9: Nina set MOCK_REAL_PASSED then MOCKING -> MARKETING (should pass)"
...
echo "Scenario 10: Mingkai MARKETING -> OFFERED"
...
echo "Scenario 11: Mingkai -> PLACED missing startDate (should fail)"
...
echo "Scenario 12: Mingkai -> PLACED with startDate (should pass)"
